...
-   **images** — Ordner für Bilder, die in Docs verwendet werden.
-   **js** — Dieser Ordner enthält alle Scriptdateien für Alpine-Komponenten, die asynchron nachgeladen werden.
-   **vendor** — Hier liegen die 3rd-party Javascripte
-   **tailwind.css** — Dies ist die Haupt CSS Datei des Designsystems. In ihr werden alle Utilityklassen des tailwind.css Frameworks sowie die
    Benutzerdefienierten CSS Klassen aus den Da
...
| `config.js`          | Diese Datei enthält Pfade zu verschiedenen Ordnern im Designsystem. Sie wird derzeit vor allem von verschiedenen Gulp Tasks verwendet. Sie kann beliebig erweitert werden und überall da genutzt werden, wo in einem Javascript auf bestimmte Pfade zugegriffen werden soll.                          |
