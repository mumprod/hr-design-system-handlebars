...
import testInclude from './data/test-include.json'
const JSONIncluder = require('scripts/jsoninclude.js')
const testTeaser = JSONIncluder.parse(JSON.stringify(testInclude))
console.log(testTeaser)
...
<Preview withToolbar>
    <Story name="Standard Hero" args={testTeaser}>
        {Template.bind({})}
    </Story>
