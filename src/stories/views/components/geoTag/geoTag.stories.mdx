import {
    ArgsTable,
    Meta,
    Story,
    Canvas,
    Preview,
    ColorPalette,
    ColorItem,
} from '@storybook/addon-docs'

import geoTag from './geoTag.hbs'

<Meta
    title="Komponenten/geoTag"
    argTypes={{
        _text: {
            control: 'text',
            description: 'Beschriftung des geoTag',
        },
    }}
                decorators={[
        (Story) => {
            return `<div class="relative py-8">  
                ${Story()} 
                </div>`
        },
    ]}
/>

export const Template = ({...args }) => {
    // You can either use a function to create DOM elements or use a plain html string!
    // return `<div>${geoTag}</div>`;
    return geoTag({...args })
}

# geoTag

<Preview withToolbar>
    <Story
        name="Frankfurt"
        args={{
            _text: 'Frankfurt'
        }}
    >
        {Template.bind({})}
    </Story>
</Preview>

## Einbindung

Die Komponente wird in Form eines Handlebar Partials eingebunden.

```html
{{> components/geotTag/geoTag.hbs text="Frankfurt"}}
```

Mit folgenden Parametern kann die Komponente konfiguriert werden.

| Parameter  | Typ      | Mögliche Werte                                                                                                                                                                                                                                                     | Erläuterung                                      |
| :--------- | :------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------- |
| **\_text** | `String` | Beliebiger Text                                                                                                                                                                                                                                                    | Die Beschriftung des geoTags.                    |
